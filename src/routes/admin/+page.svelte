<script lang="ts">
	...
	const login: () => Promise<void> = async () => {
		...
		if (res.ok) {
			...
			toast(`${data.messageload.error}`, { icon: '❌', duration: 1000 });
		}
	};
		...
		<form onsubmit={handleSubmit}>
			<div class="flex flex-col items-center justify-center">
				<input
					...
					bind:value={username}
					placeholder="아이디"
					class="mb-2 w-full cursor-pointer rounded-lg border border-pink-200 bg-white py-3 transition duration-300 hover:shadow-xl hover:shadow-pink-300 focus:scale-105 focus:ring-2 focus:ring-pink-400 max-[480px]:text-sm"
				/>
				...
				<input
					...
					type="password"
					placeholder="비밀번호"
					class="mb-3 w-full cursor-pointer rounded-lg border border-blue-200 bg-white py-3 transition duration-300 hover:shadow-xl hover:shadow-blue-300 focus:scale-105 focus:ring-2 focus:ring-blue-400 max-[480px]:text-sm"
				/>
				...
				<button
					...
					type="submit"
					class="w-full cursor-pointer rounded-lg bg-gradient-to-r from-pink-400 to-blue-400 py-3 font-bold text-white shadow-xl transition duration-300 hover:shadow-xl hover:shadow-pink-300 focus:scale-105 active:scale-120 max-[480px]:text-sm"
				>
					💕 로그인 💕
				</button>
			</div>
