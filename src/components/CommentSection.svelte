...
<div class="rounded-lg border bg-white/80 p-6 shadow-xl">
	...
	<div class="mb-6 space-y-2">
		...
		<button
			...
			class="w-full rounded bg-gradient-to-r from-pink-400 to-rose-400 py-2 font-bold text-white"
		>
			💕 메시지 남기기 💕
		</button>
	</div>
	...
	<div class="space-y-4">
		{#each comments.slice(0, visibleCount) as comment (comment.id)}
			<div class="rounded border bg-gradient-to-r from-pink-100 to-blue-100 p-4">
				...
				{#if editingId === comment.id}
					<textarea
						bind:value={editText}
						class="mb-2 w-full resize-none rounded border px-3 py-2"
						rows="3"
					></textarea>
					<input
						type="password"
					...
					<div class="flex space-x-2">
						<button
							...
							class="flex-1 rounded bg-green-400 py-2 font-bold text-white">저장</button
						>
						<button
							onclick={() => (editingId = null)}
							class="flex-1 rounded bg-gray-300 py-2 font-bold">취소</button
						>
					</div>
				{:else}
		...
		{#if visibleCount < comments.length}
			<button
				...
				class="mt-4 w-full rounded bg-gray-100 py-2 font-bold text-gray-600">더 보기</button
			>
